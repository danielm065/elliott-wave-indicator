"""Test 4H timeframe - find best params"""
import os
import sys
...
print(f"Loaded {len(data)} 4H assets: {sorted(data.keys())}")
print()
...
print("="*60)
print("4H OPTIMIZATION (fib=0.786, RR=1.5)")
print("="*60)
    ...
    for rsi_max in [40, 45, 50, 55, 60, 65, 70]:
        for tol in [0.05, 0.08, 0.10, 0.12, 0.15, 0.18, 0.20]:
            ...
            if ok > best_ok:
                ...
                print(f"NEW BEST: {ok} assets!")
                print(f"  zz={zz}, RSI<{rsi_max}, tol={tol}")
...
print("="*60)
print("ALL 4H ASSETS STATUS:")
print("="*60)
for asset in sorted(data.keys()):
    try:
        w, l = backtest(data[asset], best_params['zz'], best_params['rsi'], best_params['tol'], 1.5)
        if w + l == 0:
