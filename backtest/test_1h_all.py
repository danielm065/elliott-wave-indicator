"""
1H Timeframe - Find best possible result
"""
import sys
...
def main():
    print(f"\n1H - Finding best possible result...", flush=True)
    ...
    for i, (zz, fib, tol, rsi, gap, trend) in enumerate(combos):
        ...
        if passing > best_passing:
            ...
            print(f"  NEW BEST: {passing}/20 at combo {i}", flush=True)
        ...
        if (i + 1) % 500 == 0:
            print(f"  {i+1}/{len(combos)} tested (best so far: {best_passing})...", flush=True)
    
    best_results.sort(key=lambda x: (-x['passing'], -x['with_trades']))
    
    print(f"\n{'='*70}", flush=True)
    print(f"BEST RESULT: {best_passing}/20", flush=True)
