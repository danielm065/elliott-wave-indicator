"""
Try without trend filter to get more signals
"""
import sys
...
# Grid with no trend filter, trying to get more signals
ZZ_VALUES = [2, 3]
RR_VALUES = [0.5, 0.75, 1.0]
RSI_VALUES = [50, 60, 70, 80]  # Higher = more signals
TOL_VALUES = [0.15, 0.20, 0.25, 0.30, 0.35]
SIGNAL_GAP = [3, 5, 7]
...
def main():
    print(f"\nTarget: 16/20 assets above 80% WR (no trend filter)", flush=True)
    ...
    combos = list(product(ZZ_VALUES, RR_VALUES, RSI_VALUES, TOL_VALUES, SIGNAL_GAP))
    ...
    print(f"\n{'='*70}", flush=True)
    print(f"TOP RESULTS (no trend filter)", flush=True)
    print(f"{'='*70}", flush=True)
    
    if not best_results:
        print("No good results found", flush=True)
        return
